...
function FramesProductCard({ product }: { product: Product }) {
  const { t } = useLocale();
...
export default function FramesCatalogue() {
  ...
  const title = dict.destinations.frames.title;
  const lead = dict.destinations.frames.lead;
  ...
  return (
    <div className="frames-page">
      <section className="frames-hero" aria-label={title}>
        {reduceMotion ? (
          <Image
            src="/images/premium-video-poster.jpg"
            alt=""
            fill
            priority
            sizes="100vw"
          ...
          <video
            ...
            playsInline
            preload="metadata"
            poster="/images/premium-video-poster.jpg"
          >
            <source src="/videos/premium-frames.mp4" type="video/mp4" />
          </video>
        )}
      ...
      <section className="frames-catalogue wrap">
        ...
        {loading ? (
          ...
          <div className="frames-product-grid">
            {frames.map((product) => (
              <FramesProductCard key={product.id} product={product} />
            ))}
          </div>
        )}
      </section>
