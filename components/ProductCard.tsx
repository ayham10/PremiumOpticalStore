...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[var(--radius)] border border-[var(--line)] bg-white shadow-[var(--shadow-soft)] transition duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow)]">
      <Link href={`/product/${product.slug}`} className="relative block aspect-[4/5] overflow-hidden bg-[var(--mist)]">
        <Image
          src={image}
          alt={product.name}
          fill
        ...
        />
        <span className="absolute left-3 top-3 pill bg-white/90">{product.category}</span>
      </Link>
      <div className="flex flex-1 flex-col gap-2 p-5">
        <div className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--slate)]">
          {product.brand}
        </div>
        <Link href={`/product/${product.slug}`} className="font-[family-name:var(--font-display)] text-2xl">
          {product.name}
        </Link>
        ...
        <div className="mt-auto flex items-center justify-between pt-4">
          ...
          <Link href={`/product/${product.slug}`} className="btn btn-ghost !min-h-10 !px-4 !text-sm">
            View
          </Link>
        </div>
      </div>
