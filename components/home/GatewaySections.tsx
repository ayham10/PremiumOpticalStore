    ...
    <div>
      {SECTIONS.map((section, index) => (
        <Link
          key={section.key}
          href={section.href}
          className="group relative block min-h-[78svh] overflow-hidden text-white"
        >
          <Image
            src={section.image}
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/82 via-black/35 to-black/20 transition group-hover:from-black/88" />
          ...
          <motion.div
            ...
            className="relative z-10 flex min-h-[78svh] items-end"
          >
            <div className="wrap w-full py-16 md:py-20">
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-white/70">
                {t(`home.gateway.${section.key}.subtitle`)}
              </p>
              <h2 className="mt-3 max-w-xl font-[family-name:var(--font-display)] text-[clamp(2.4rem,6vw,4.4rem)] leading-[1.02]">
                {t(`home.gateway.${section.key}.title`)}
              </h2>
              <span className="btn btn-glass pointer-events-none mt-8 inline-flex">
